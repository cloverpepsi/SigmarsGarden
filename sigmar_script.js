...
const PROD_BITBOARDS = "00000000110000000011110000000111100110011110011110010000111000000011110010000110011110000000111100000011110000000110000000000000 00000000111000000010010000000010100110010010011110110001101100000011110110000111011110000001101100000011110000000111000000000000 00000000111000000011110000000111100110011110011010110001001100000011010110000111011010000001001100000011010000000111000000000000 00000000010000000011110000000111000110011110011110010000111100000011110110000110011110000000111100000011110000000110000000000000 00000000111000000011110000000111100110011110011010110001101100000010110110000011011010000001101100000010110000000011000000000000 00000000011000000010110000000110000110010110011110010001101100000011110110000111011110000001101100000011110000000111000000000000 00000000111000000011110000000011100110011010011100110001111000000011100000000111001110000001111100000011110000000111000000000000 00000000111000000011110000000111100110010110011110010001110000000011110010000110010110000000111100000011110000000111000000000000 00000000110000000011010000000111100110011110011110110000001100000011110110000111011110000001101100000011010000000110000000000000 00000000111000000010010000000110100110010110011110110001110100000011010110000111010110000001011100000011110000000111000000000000 00000000110000000011010000000111100110010110011110010000111000000011110010000111011110000001111100000011110000000110000000000000".split(" ")
...
const PROD_BOARD_STRING = "00000000111000000011110000001111100111011110011110111001111100000011110111000111011110000001111100000011110000000111000000000000"
const WIDTH = 1050;
const HEIGHT = 850;
...
const SELECTOR_PATH = new Path2D();
SELECTOR_PATH.rect(820,885,200,50)
...
class Variant {
    constructor(hex_compat = function(h1, h2){ return true; },
                atom_compat = function(a1, a2){ return true; },
                is_selectable = function(h){ return true; },
        ...
        this.board_details = board_details
...
function drawShape(x, y, r, sides, rot=0, color="#BBB") {
  ctx.save()
  ctx.beginPath();
  ctx.translate(x, y);
  for (let i = 0; i < sides; i++) {
    const rotation = ((Math.PI * 2) / sides) * i + rot;
    if (i === 0) {
...
function screenPosition(r, q){
    return [80*(q-5)+40*(r-5)+Math.round(WIDTH/2),69*(5-r) + Math.round(HEIGHT/2)];
}
...
// COLVAN VARIANT DEFINITION

const COLVAN = new Variant(
    hex_compat = function(h1, h2) { return true; },
    atom_compat = function(a1, a2) {
    ...
    board_gen = function(){
        ...
        // Add metal matches to the board

        let metals = [2, 2, 2, 2, 1]

        for (let i = 4; i >= 0; i--)
        {
            for (let j = 0; j < metals[i]; j++)
                {
                    metalBag.push([METALS[i], METALS[i]])
                }
        }
    ...
    info_board = function(x=null, y=null, click=false){
        ...
        if (showInfo){
            ...
            ctx.fillText("The four cardinal elements match", left_pos+120, render_y+170)
            ctx.fillText("with others of the same type.", left_pos+120, render_y+170+22)
            ...
            ctx.fillText("Salt matches with any of the", left_pos+530, render_y+170)
            ctx.fillText("cardinal elements, or with itself.", left_pos+530, render_y+170+22)
            ...
            for (const atype of METALS){
                atype.render([info_x+(atype == GOLD ? 15 : 0),render_y+(atype == GOLD ? 50 : 0)])
                if (atype != GOLD) { 
                    atype.render([info_x,render_y+100]);
                    ctx.fillText("+",info_x,render_y+56)
                    ctx.fillText("➤", info_x+50, render_y+56)
                }
                info_x += 100
            }

            ctx.fillText("In the House Colvan variant of Sigmar's Garden, the metals match", left_pos+250, render_y+170)
            ctx.fillText("with others of the same type instead of with quicksilver.", left_pos+250, render_y+170+22)
        } else {
            ctx.clearRect(0,1060,WIDTH,HEIGHT)
        }
...
const PRODUCTION = new Variant(
    hex_compat = function(hexA, hexB) { 

        let leftHex = hexA[1] + hexA[0] < hexB[1] + hexB[0] ? hexA : hexB;
        let rightHex = hex_equals(leftHex, hexB) ? hexA : hexB;

        if ((leftHex[1] + leftHex[0]) < 10 && (rightHex[1] + rightHex[0]) > 10 && leftHex[0] < 5 && rightHex[1] > 5) return false;
    ...
    board_gen = function(){
        ...
        let bitboard = PROD_BITBOARDS[RandomInt(PROD_BITBOARDS.length)]
        ...
        while (!boardGenerated)
        {
            ...
            {
                // find all marbles that could be chosen for the next move
                choosableMarbles = temporaryMarbles.filter(x => hexIsChoosable(x, temporaryMarbles) && !hex_equals(x, center))
                // choose the next move
                if (choosableMarbles.length >= 2)
                {
                    let marbleA = [100, 100]
                    let marbleB = [100, 100]
                    let MarbleAs = [... choosableMarbles];
                    
                    while (MarbleAs.length >= 1) {

                        // choose a random marble A to check
                        marbleA = MarbleAs[RandomInt(MarbleAs.length)];
                        remove(MarbleAs, marbleA);

                        // iterate over all possible chooseable other marbles to make sure one is choosable
                        let MarbleBs = [];
                        for (const hex of choosableMarbles)
                        {
                            if (this.hex_compat(marbleA, hex)) {MarbleBs.push(hex);}
                        }
                        remove(MarbleBs, marbleA);

                        if (MarbleBs.length > 0) 
                        {
                            marbleB = MarbleBs[RandomInt(MarbleBs.length)];
                            break;
                        }
                    }

                    if (!hex_equals(marbleB,[100,100])) {
                        moveHistory.push([marbleA, marbleB]);
                ...
                else if (hexIsChoosable(center, temporaryMarbles) && hex_selected(center[0], center[1], temporaryMarbles))
                {
                    // only option is to choose Gold as our next move
                    moveHistory.push([center, center]);
    ...
    post_click = function(h){
        if (selected_hexes.length == 0){
            ...
            if (hex_selected(h[0],h[1])) {
                ...
                if (this.atom_compat(get_atom(h), get_atom(selected_hexes[0])) && this.hex_compat(h, selected_hexes[0])) {
                    atom_field[h[0]][h[1]] = null;
                    atom_field[selected_hexes[0][0]][selected_hexes[0][1]] = null;
    ...
    info_board = function(x=null, y=null, click=false){
        ...
        if (showInfo){
            ...
            ctx.fillText("Select a free marble, and then pick a matching marble to remove them both from the board.",WIDTH/2,1100)
            ctx.fillText("Marbles in different chambers can only be matched if a conduit is connecting the two chambers.", WIDTH/2, 1122)
    ...
    board_details = function(first=false){


        if (first) {
            drawShape(WIDTH/2, HEIGHT/2, 475, 6, 0, "#532")
            let hexList = [[8,2],[2,5],[5,8]]
            for (const hex of hexList) {
                let boardCoords = screenPosition(hex[0], hex[1])
                drawShape(boardCoords[0], boardCoords[1], 220, 6, 0, "#BBB")
            }

        }
        else {
            let conduitList = [[4,3,"II"],[6,2,"II"],[7,6,"I"],[8,4,"I"]]
            ...
            for (const conduit of conduitList) {

                let boardCoords = screenPosition(conduit[0], conduit[1])
                drawShape(boardCoords[0],boardCoords[1],46,6,Math.PI/6,"#543")
                drawCircle(boardCoords[0],boardCoords[1],38,"#999")
                drawCircle(boardCoords[0],boardCoords[1],33,"#222")
                ctx.fillText(conduit[2],boardCoords[0],boardCoords[1]+10);
            }
        }
...
let myGameArea = {
    ...
    clearHex : function(r,q){
        let atomPosition = screenPosition(r, q);
    ...
    clear : function() {
        ...
        for (let i = 0; i < 16; i++)
        {
            ...
            {
                let num = i * 8 + j;
                if (gamemode.board_string[num] == "1")
                {
                    // add hex
                    let q = Math.floor(num / 11);
                    let r = (num % 11);
    ...
    drawAtoms : function(){
        ...
        localStorage.setItem("gamemode", getKeyByValue(VARIANT_LIST,gamemode))

        ctx.font = "25px Lexend";
        ctx.fillStyle = "white";
        ctx.textAlign = "center";

        ctx.fillText("Wins: "+wincount.toString(),920,990)

        ctx.fillStyle = "#777"
        ctx.fill(SELECTOR_PATH)
        ctx.fillStyle = "white";
        ctx.fillText(getKeyByValue(VARIANT_LIST,gamemode),920, 920)

    },
}

function elementFromCoords(x,y){

    if (ctx.isPointInPath(SELECTOR_PATH,x,y)) { return 'switch'; }
...
canvas.addEventListener('click', function(event){
    ...
    let location = elementFromCoords(mouseX, mouseY)
    ...
    else if (location == 'switch'){
        gamemode = VARIANT_ORDER[(VARIANT_ORDER.indexOf(gamemode) + 1) % VARIANT_ORDER.length]
        myGameArea.reset()
    }
